...
(defvar org-taskjuggler--auto-number 1
  "Counter for automatically assigning numbers to TaskJuggler tasks.")
...
taskreport plan \"\" {
  headline \"%title\"
  columns name, start, end, duration, chart { width 1700 }
  loadunit shortauto
  hideresource 1
...
a unique id will be associated to it."
  (let* ((allocate (org-element-property :ALLOCATE task))
    ...
    ;;(cl-incf org-taskjuggler--auto-number)
    (concat
     ;; Opening task.
     (format "task %s \"%s\" {\n"
     ...
     ;; Add task number
     ;;(format "  tag tjorder-%03d\n" org-taskjuggler--auto-number)
  ...
  (let* ((full-name (subst-char-in-string ?/ ?\\ (file-truename file)))
    ...
    (save-window-excursion
      (let ((outbuf (get-buffer-create "*Org Taskjuggler Output*")))
        ...
        (message (format "Processing TaskJuggler file %s to %s..." (shell-quote-argument full-name) (shell-quote-argument out-dir)))
        (message
         (replace-regexp-in-string
